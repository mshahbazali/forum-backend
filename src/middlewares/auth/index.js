...
const checkUserName = async (req, res, next) => {
    await authSchema.findOne({ userName: req.body.userName }).then(async (payload) => {
        if (req.body.referenceId == "login") {
            if (payload) {
                const token = await jwt.sign({ _id: payload._id }, jwtKey);
                await bcrypt.compare(req.body.password, payload.password, function (err, result) {
                    if (result) {
...
module.exports = { validateInfo, hashPassword, checkUserName, getUserInfoFromToken }